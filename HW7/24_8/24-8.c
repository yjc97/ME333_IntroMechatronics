...
static volatile int REFarray[PLOTPTS]; // reference values to plt
static volatile int StoringData =   0; // if this flag = 1, currently storing plot data
static volatile float Kp = 0.2, Ki = 0.1; // control gains
static volatile int pwm[PLOTPTS];
...
void __ISR(_TIMER_2_VECTOR, IPL5SOFT) Timer2ISR(void) {
  ...
  OC1RS = (unsigned int)(unew * PR3 /100.0);
...
int main(void) {
  ...
  makeWaveform(); 
  // init adc
  AD1CON1bits.SSRC = 0b111; // Set the conversion process to be automatic
